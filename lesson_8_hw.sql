...
SELECT p.user_id, p.birthday, l.target_id , l.user_id, IF (l.target_type_id = 2, 2, NULL) AS like_target
  FROM profiles p
  	LEFT JOIN likes l 
...
-- вот этот вариант работает, но только без группировки, поэтому результат неверный. Когда добавляю GROUP BY по дню рождения выдает ошибку

SELECT COUNT(like_target) FROM
(SELECT p.birthday, l.target_id , l.user_id, IF (l.target_type_id = 2, 2, NULL) AS like_target
  FROM profiles p
  	LEFT JOIN likes l 
  	ON l.target_id = p.user_id
ORDER BY p.birthday DESC
...
-- выдает ошибку
SELECT COUNT(like_target) FROM
(SELECT  COUNT(p.user_id), IF (l.target_type_id = 2, 2, NULL) AS like_target
  FROM profiles p
  	LEFT JOIN likes l 
  	ON l.target_id = p.user_id
GROUP BY p.birthday
