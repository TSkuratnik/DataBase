...
WHERE 
	...
	AND target_id IN (SELECT profiles.user_id, birthday 
FROM profiles
ORDER BY profiles.birthday DESC 
LIMIT 10);

-- не знаю как дату эту поставить кодом
SELECT  
	COUNT(target_id)
FROM likes 
WHERE 
	target_type_id = (SELECT id FROM target_types WHERE name = 'users')
	AND 
	(SELECT birthday FROM profiles WHERE profiles.user_id = likes.target_id) > '2015-10-18';
